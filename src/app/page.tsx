  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden">
        <div aria-hidden="true" className="pointer-events-none absolute -right-40 -top-40 h-[34rem] w-[34rem] rounded-full bg-bay-200/40 blur-3xl" />
        <div aria-hidden="true" className="pointer-events-none absolute -left-32 top-64 h-80 w-80 rounded-full bg-coral-200/40 blur-3xl" />
        <div className="container-page relative grid items-center gap-12 pb-16 pt-12 lg:grid-cols-[1.1fr_1fr] lg:pb-24 lg:pt-20">
          <div className="animate-fade-up">
            <p className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1 text-xs font-semibold text-navy-600 shadow-sm ring-1 ring-navy-100">
              <MapPin className="h-3.5 w-3.5 text-coral-500" aria-hidden="true" /> Berkeley · Albany · El Cerrito · East Bay
            </p>
            <h1 className="mt-5 text-4xl font-extrabold leading-[1.05] sm:text-5xl lg:text-6xl">
              Local jobs. <span className="text-bay-500">Real experience.</span> <span className="relative whitespace-nowrap">Built for teens.<svg aria-hidden="true" viewBox="0 0 300 12" className="absolute -bottom-2 left-0 h-3 w-full text-coral-400"><path d="M2 9c60-6 140-8 296-3" stroke="currentColor" strokeWidth="4" fill="none" strokeLinecap="round" /></svg></span>
            </h1>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-3 text-sm">
              <Link href="/auth/sign-up?role=teen" className="btn-navy">I&apos;m a teen — create a profile</Link>
          ...
          <div className="relative hidden h-[30rem] lg:block" aria-hidden="true">
            <SafeImage src={IMAGES.hero} alt="" className="absolute right-0 top-0 h-80 w-[82%] rounded-4xl shadow-lift" />
            <SafeImage src={IMAGES.heroAlt} alt="" className="absolute bottom-0 left-0 h-56 w-[56%] rounded-4xl border-8 border-cream-100 shadow-lift" />
            <div className="absolute bottom-10 right-6 w-60 animate-fade-up rounded-2xl bg-white p-4 shadow-lift [animation-delay:300ms]">
              <p className="text-xs font-semibold uppercase tracking-wider text-coral-600">Application sent</p>
              <p className="mt-1 text-sm font-semibold">After-school dog walker</p>
              <p className="text-xs text-navy-500">Delivered to the employer&apos;s dashboard</p>
